...
int main() {
	...
	printf("Nome: %s\n", nome);
	printf("GRR: %u\n", getGRR());
	printf("Entre com o tamanho do vetor a ser testado e com o valor a ser buscado: ");
	...
	*numComps = 0;
	for (int i = 0; i < size; i++)
		vetorTeste[i] = i;
	start = clock();
	int posValSearch = buscaSequencialPossivel(vetorTeste, size, valSearch, numComps);
	end = clock();
