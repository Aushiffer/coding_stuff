...
long part(int vetor[], int a, int b, int* indPivo) {
	if (a >= b)
		return 0;
	...
	return comps; // retorna a posição do pivô selecionado no vetor
}
...
long quickSortPossivel(int vetor[], int a, int b) {
	if (a >= b)
		return 0;
	int indPivo;
	long comps = part(vetor, a, b, &indPivo); // adquire o índice do pivô (na atual cham. recursiva)
	quickSortPossivel(vetor, a, indPivo); // chama o quick sort de a até indPart no vetor
	quickSortPossivel(vetor, indPivo + 1, b); // chama o quick sort para o resto do vetor

	return comps; // retorna as comparações feitas no algoritmo
}

long quickSort(int vetor[], int tam) {
	return quickSortPossivel(vetor, 0, tam); // wrapper as comparações feitas no quick sort
}
