...
long quickSortPossivel(int vetor[], int a, int b) {
	if (a >= b)
		return 0;
	int indPart = part(vetor, a, b); // adquire o índice do pivô (na atual cham. recursiva)
	quickSortPossivel(vetor, a, indPart); // chama o quick sort de a até indPart no vetor
	quickSortPossivel(vetor, indPart + 1, b); // chama o quick sort para o resto do vetor

	return 0; // retorna as comparações feitas no algoritmo
}
