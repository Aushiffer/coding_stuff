...
int removeItem(struct listaDE *l, int valor) {
	...
	while (ptrl -> proximo != NULL) {	
		if (ptrl -> valor == valor) {
			...
			// ^ com isso, agora o ptr. aux. não está mais encadeado na LDE
			free(ptrl);
...
int insereOrdenado(struct listaDE *l, int x) {
	if (l -> cabeca -> valor >= x) {
		insereCabeca(l, x);
		return 1;
	} else if (l -> cauda -> valor <= x) {
		insereCauda(l, x);
		return 1;
	} // verifica se o item pode ser inserido diretamente na cauda ou na cabeça
	struct item *novoItem = criaItem(x);
	struct item *ptrl = l -> cabeca; // criação do ptr. aux. e novo item
	while (ptrl -> proximo != NULL) {
		if (ptrl -> valor <= x && ptrl -> proximo -> valor >= x) {
			novoItem -> anterior = ptrl; // anterior do novo item é o ptr. aux.
			novoItem -> proximo = ptrl -> proximo; // próximo do novo item é o próximo do ptr. aux
			ptrl -> proximo = novoItem; // próximo do ptr. aux. é o novo item
			ptrl -> proximo -> anterior = novoItem; // próximo do anterior do ptr. aux. é o novo item
			(l -> tamanho)++;
			return 1;
		}
		ptrl = ptrl -> proximo; // atualização do ptr. aux.
	}
