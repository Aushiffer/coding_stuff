...
void duel(dl_list *conc_l, dl_list *prt_l, node_t *cnd, int *match, int *mismatch) {
	int vict_cnd = 0, vict_prt = 0;
	short atrs_cnd = 0, atrs_prt = 0;
	node_t *prt = new_node();
...
void pairing(stack_t *cnd_s, dl_list *conc_l, dl_list *prt_l, int *match, int *mismatch, int *no_pair) {
	while (!empty_stack(cnd_s)) {
		...
		if (size_list(conc_l) > 1) {
			printf("\nThere is more then one pretendent compatible with the candidate %s. Initiating dispute...\n\n", cnd_aux->name);
			int p, min_relevance;
			p = size_list(conc_l);
			aux = conc_l->head;
			...
			(*no_pair)++;
                        printf("\n%s is leaving the program without a pair...\n", cnd_aux->name);
		}
