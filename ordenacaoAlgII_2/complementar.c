...
void printVetorUnsigned(unsigned int *vetor, int tam) {
	for (int i = 0; i < tam; i++)
		printf("%d ", vetor[i]);
	printf("\n");
}

void aleatorizarVetorUnsigned(unsigned int *vetor, int tam) {
	for (int i = 0; i < tam; i++)
		vetor[i] = rand() % RNG_CAP;
}
...
void introSort(int *vetor, int a, int b, int profundidade, unsigned long *comps) {
	...
	if (b < 64) {
		...
		int pivo = particaoQS(vetor, a, b, comps);
		introSort(vetor, a, pivo, profundidade - 1, comps);
		introSort(vetor, pivo + 1, b, profundidade - 1, comps);
	}
} /* ^ Vetor pequeno: faz Insertion Sort
   * Recursão muito profunda: faz Heapsort
   * Nenhum dos anteriores: faz Quicksort */ 