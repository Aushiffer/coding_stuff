...
void countingSort(unsigned int vetor[], int tam, int maxVal) {
	...
	if (!(vetorContagem = (unsigned int *)calloc(maxVal + 1, sizeof(unsigned int)))) {
		printf("[-] Impossível alocar o vetor de contagem.\n");
		return;
	} // Precisa de maxVal + 1 elementos para acomodar todos os naturais em [0..maxVal], pois inclui maxVal
	for (int k = 0; k < tam; k++)
...
unsigned long algoritmoExtra(int vetor[], int tam) {
	unsigned long comps = 0;
	int profundidade = (int)log2(tam) * 2;
	introSort(vetor, 0, tam, profundidade, &comps);
