...
unsigned int get_place(unsigned int n_machines) { return (rand() % n_machines) - 1; }

unsigned int get_attempt() { return rand() % 100 + 1; }

void print_attempt(teddy_machine *machine, unsigned int attempt) {
	if (!machine) {
		fprintf(stderr, "print_attempt(): [-] NULL machine input\n");
...
void remove_machine(teddy_machine *list, teddy_machine *rm) {
	if (!list || !rm) {
