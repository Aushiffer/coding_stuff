...
int main() {
	...
	while (!pilhaVazia(pilhaCandidatos)) {
		...
		while (k < tamLista(listaPretendentes)) {
			...
			if (relevanciaPretendente >= 1) {
				...
				printf("[+] O PRETENDENTE '%c' TEM ALGUM ATRIBUTO EM COMUM COM '%c'! ELE VAI CONCORRER AO MATCH!\n\n", aux -> id, idCand);
				...
				printf("[-] O PRETENDENTE '%c' FOI DE BASE POIS NÃO COMBINOU COM '%c'! CONTINUA NA LISTA DE PRETENDENTES...\n\n", aux -> id, idCand);
				aux = aux -> prox;
			}
			k++;
		}
	...
	destroiLista(listaConcorrentes, &match);
