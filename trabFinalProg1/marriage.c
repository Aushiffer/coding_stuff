...
int pretendenteValido(struct pessoa *prt) {
	int numAtrs = 0;
	for (int i = 0; i < MAX_ATR; i++)
		if (prt->atr[i] == 1)
			numAtrs++;
	...
	while (!pilhaVazia(pilhaCandidatos)) {
		...
		for (int k = 0; k < j; k++) {
			...
			if (relevanciaPretendente > 1) {
				...
				conc->relevancia = aux->relevancia;
				aux = aux->prox;
		...
		if (tamLista(listaConcorrentes) > 1) {
			...
			for (int n = 0; n < j - 1; n++) {
				...
				struct pessoa *perdedor = criaPessoa(aux->id, aux->atr[0], aux->atr[1], aux->atr[2], 
									aux->atr[3], aux->atr[4], aux->atr[5]);
				delist(listaConcorrentes, minRelevancia);
				enlist(listaPretendentes, perdedor);
			}
			...
			aux = listaConcorrentes->cbc;
			printf("\n[+] '%c' GANHOU! ELE VAI CONCORRER AO MATCH!\n\n", aux -> id);
			printf("-= INÍCIO DO DUELO CANDIDATO X PRETENDENTE =-\n\n");
			struct pessoa *conc = criaPessoa(aux->id, aux->atr[0], aux->atr[1], aux->atr[2],
								aux->atr[3], aux->atr[4], aux->atr[5]);
			...
			printf("-= FIM DO DUELO CANDIDATO X PRETENDENTE =-\n\n");
